...
// Async variant — yields the event loop every CHUNK lines so the renderer doesn't freeze
// while walking export_descr_unit.txt (often 50k+ lines).
const EDU_CHUNK = 3000;
const eduTick = () => new Promise(r => setTimeout(r, 0));
export async function parseEDUAsync(text) {
  const lines = text.split(/\r?\n/);
  const out = [];
  let cur = null;
  for (let i = 0; i < lines.length; i++) {
    ...
    if ((i % EDU_CHUNK) === 0 && i > 0) await eduTick();
  }
  if (cur) out.push(cur);
  return out;
}
